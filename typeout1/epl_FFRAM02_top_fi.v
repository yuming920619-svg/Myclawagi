/******************************************************************************
* File Name   : epl_FFRAM02_top_fi.v
* Created     : 2026/01/24
* Author      : EPL Lab Memory Test and Repair Group (FI extension)
* Version     : 1.0
*
* Description :
*   Top wrapper of FFRAM02 memory model with *synthesizable* fault injection
*   hooks for ECC verification.
*
*   Implemented FI modes (using existing pFS_i only):
*     - Normal mode      : pFS_i = 0 (no injection)
*     - Write failure    : assert pFS_i during write command (nCEN_i=0 & nWEN_i=0)
*                          -> corrupt codeword *before* it enters Column_Access.
*     - Read disturb     : assert pFS_i during read command  (nCEN_i=0 & nWEN_i=1)
*                          -> pFS_i is latched and later flips selected bit(s)
*                             *after* Read_Mux and before ECC decoder.
*
*   Address targeting is controlled by compile-time defines (word masks) to avoid adding
*   extra top-level ports (tape-out friendly).
*
******************************************************************************/
