...
// Module Name      : epl_testbench_rtl_fi
// Version          : 1.0
//
// Description      : RTL Testbench for epl_FFRAM02_top_fi
//                    - Adds 3 test modes:
//                        1) Normal          (no FI)
//                        2) Read disturb    (FI on read)
//                        3) Write failure   (FI on write)
//==============================================================================
`timescale 1ns/1ps
...
//==== ���� AVC �ɮ׼��Y ====//
initial begin
    avc_file = $fopen("pattern.avc"); // �إ��ɮ�

    // �g�J FORMAT �w�q
    // ����: CLK, RST, CEN, WEN, A(Bus), D(Bus), Q(Bus)
    $fwrite(avc_file, "FORMAT CLK, RST, CEN, WEN");

    // �i�} Address Pins
    for (bit_idx = `ADDR_WIDTH-1; bit_idx >= 0; bit_idx = bit_idx - 1)
        $fwrite(avc_file, ", A[%0d]", bit_idx);

    // �i�} Data Input Pins
    for (bit_idx = `WORD_WIDTH-1; bit_idx >= 0; bit_idx = bit_idx - 1)
        $fwrite(avc_file, ", D[%0d]", bit_idx);

    // �i�} Data Output Pins
    for (bit_idx = `WORD_WIDTH-1; bit_idx >= 0; bit_idx = bit_idx - 1)
        $fwrite(avc_file, ", Q[%0d]", bit_idx);

    $fwrite(avc_file, ", DiXor");
    $fwrite(avc_file, ", DoXor");
    $fwrite(avc_file, ", ERR");
    $fwrite(avc_file, ";\n"); // ���� FORMAT ��
end
...
always @(posedge pCLK_r) begin
    // 1. �g�J�򥻱���T��
    // R1: ����1��, std: �ɧǦW��
    // CLK �o�̶� 1 (�]���O�b posedge Ĳ�o�A�N�����ɥ� 0 �� 1)
    $fwrite(avc_file, "R1 std 1 %b %b %b ", nRST_r, nCEN_r, nWEN_r);

    // 2. �g�J Address (Input: 0/1)
    for (bit_idx = `ADDR_WIDTH-1; bit_idx >= 0; bit_idx = bit_idx - 1) begin
        $fwrite(avc_file, "%b", pA_r[bit_idx]);
    end
    $fwrite(avc_file, " "); // �[�ťդ��j��K�\Ū

    // 3. �g�J Data Input (Input: 0/1)
    for (bit_idx = `WORD_WIDTH-1; bit_idx >= 0; bit_idx = bit_idx - 1) begin
        $fwrite(avc_file, "%b", pD_r[bit_idx]);
    end
    $fwrite(avc_file, " ");

    // 4. �g�J Data Output (Expect: L/H/X)
    // �ھ� PDF �����G0��L, 1��H, ��L��X
    for (bit_idx = `WORD_WIDTH-1; bit_idx >= 0; bit_idx = bit_idx - 1) begin
        if (pQ_w[bit_idx] === 1'b0)
            $fwrite(avc_file, "L");
        else if (pQ_w[bit_idx] === 1'b1)
            $fwrite(avc_file, "H");
        else
            $fwrite(avc_file, "X"); // ���w�ȩΰ����ܵ��� Don't Care
    end


    $fwrite(avc_file, " ");

    // 4b. �g�J DiXor/DoXor/ERR (Expect: L/H/X)
    if (pDiXor_w === 1'b0)
        $fwrite(avc_file, "L");
    else if (pDiXor_w === 1'b1)
        $fwrite(avc_file, "H");
    else
        $fwrite(avc_file, "X");
